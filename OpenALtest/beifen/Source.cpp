...
void main()
{
	...
	if (EnableSource2){
		...
		ALuint buffer2 = alutCreateBufferFromFile("E:\\VS2013\\Projects2013\\OpenALtest\\OpenALtest\\10217.WAV");
		alSourcei(source1[1], AL_BUFFER, buffer2);
		alSourcePlay(source1[1]);
		alSourcei(source1[1], AL_LOOPING, AL_FALSE);//第二个音频是否循环
	}
	...
	alSourcef(source1[0], AL_MAX_GAIN, 1.f);//设置AL_MAX_GAIN，即使设得比1大，也会自动回成1,并报输出ERROR
	...
	} while (state == AL_PLAYING && tick<=4000);
