...
//ALfloat orivec[] = { lis.at.x, lis.at.y, lis.at.z,
//lis.up.x, lis.up.y, lis.up.z };
//alListenerfv(AL_ORIENTATION, orivec);
void ModifyAngle(ALfloat *ary){
	cout << "打印Orivec元素: " << ary[0] << endl;
	if (ary[0] == 180){
		ary[0] = 0;
	}
}
...
void main()
{
	...
	*/
	...
	ALfloat orivec[] = { 0.f, 0.0f, -1.0f, 0.0f, 1.0f, 0.0f };
	//ModifyAngle(orivec);
	...
	do{
		...
		////旋转音锥
		//ALfloat updatex = -sinf(play_it*PI / 180.f);
		//ALfloat upadtez = cosf(play_it*PI / 180.f);
